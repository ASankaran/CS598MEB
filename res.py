...
class ResNet(nn.Module):
	def __init__(self, block, layers, num_classes=len(classifications)):
		super(ResNet, self).__init__()
		self.in_channels = 2
		self.conv = conv3x3(1, 2)
		self.bn = nn.BatchNorm1d(2)
		self.relu = nn.ReLU(inplace=True)
		self.layer1 = self.make_layer(block, 2, layers[0])
		self.layer2 = self.make_layer(block, 4, layers[1], 2)
		self.layer3 = self.make_layer(block, 8, layers[2], 2)
		self.avg_pool = nn.AvgPool1d(8)
		self.fc = nn.Linear(1432, num_classes)
...
class ResMutationNet(ResNet):
	def __init__(self):
		super().__init__(ResidualBlock, [2, 2, 2])
